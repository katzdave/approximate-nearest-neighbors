...
A key limitation of nearest neighbors search algorithms is that the relevance of each dimension is built into the index.  In this thesis we present a method for supporting efficient approximate nearest neighbor queries in which the relevance of each dimension can be specified for each query.  Our system constructs a large set of k-d tree indexes, and heursistically selects the trees which are best suited for a query's specified relevances, and searches these trees in parallel.  Our system was benchmarked against a conventional k-d tree and a linear search to evaluate performance.
